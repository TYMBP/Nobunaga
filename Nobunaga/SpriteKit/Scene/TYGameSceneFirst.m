...
#import "TYGameView.h"
#import "TYGameSceneSecond.h"
#import "TYGamePlayTimeManager.h"
//dev
#import "TYScenarioSceneB.h"

@implementation TYGameSceneFirst {
  UITouch *_touch;
  CGPoint _touchPoint;
  NSTimer *timer;
  float _timerCount;
...
{
  //ステージメッセージ
  SKLabelNode *stage = [_UINode setStageNo];
...
//    self.physicsWorld.gravity = CGVectorMake(0, 0);
  ...
  [_gamePlayTime resetData];
  _timeLabel = [_UINode makeTimeLabel];
  if (!self.screenFlg) {
  ...
  if (([bodyNameA isEqualToString:kFrameName]) && [bodyNameB isEqualToString:kPlayerName]) {
    ...
    if (![node childNodeWithName:kEnemyName]) {
      ...
      if (self.screenFlg && posi > 230) {
        ...
        if (!self.addDataFlg) {
          self.addDataFlg = YES;
          [_gamePlayTime addData:_timerCount];
        }
        [self.bg clear:^{
          TYGameSceneSecond *second = [[TYGameSceneSecond alloc] initWithSize:self.frame.size];
          SKTransition *next = [SKTransition crossFadeWithDuration:1];
          [self.view presentScene:second transition:next];
        }];
      } else {
        [timer invalidate];
        if (!self.addDataFlg) {
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
  ...
  _touchPoint = [_touch locationInNode:self];
  
  NSUInteger tapNum = [_touch tapCount];
