...
@implementation TYGameSceneSecond {
  CGPoint _touchPoint;
  ...
  if (([bodyNameA isEqualToString:kFrameName]) && [bodyNameB isEqualToString:kPlayerName]) {
    ...
    if (![node childNodeWithName:kEnemyName]) {
      ...
      if (self.screenFlg && posi > 230) {
        ...
        [self.bg clear:^{
          TYGameSceneThird *third = [[TYGameSceneThird alloc] initWithSize:self.frame.size];
          SKTransition *next = [SKTransition crossFadeWithDuration:1];
          [self.view presentScene:third transition:next];
        }];
      } else {
        [timer invalidate];
        if (!self.addDataFlg) {
