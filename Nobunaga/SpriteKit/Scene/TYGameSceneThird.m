...
#import "TYGameSceneThird.h"
#import "TYGameView.h"
#import "TYGameSceneForth.h"
#import "TYGamePlayTimeManager.h"

@implementation TYGameSceneThird {
  CGPoint _touchPoint;
...
//    [self.baseNode addChild:self.bg];
    ...
    //リタイア
//    SKSpriteNode *retire = [self.UINode makeRetireNode];
//    retire.name = kRetireName;
//    [self.baseNode addChild:retire];
  ...
  if (([bodyNameA isEqualToString:kFrameName]) && [bodyNameB isEqualToString:kPlayerName]) {
    ...
    if (![node childNodeWithName:kEnemyName]) {
      ...
      if (self.screenFlg && posi > 230) {
        ...
        [self.bg clear:^{
          TYGameSceneForth *forth = [[TYGameSceneForth alloc] initWithSize:self.frame.size];
          SKTransition *next = [SKTransition crossFadeWithDuration:1];
          [self.view presentScene:forth transition:next];
        }];
      } else {
        [timer invalidate];
        if (!self.addDataFlg) {
